...
var
  ...
  glDisable_C: procedure(cap: GLenum); extdecl;
  ...
  glEnable_C: procedure(cap: GLenum); extdecl;
...
var
  glStateArray: array[0..$FFFF] of boolean;
...
begin
  ...
  try
    ...
    @glDisable_C := GetGLProcAddress(LibGL, 'glDisable');
    ...
    @glEnable_C := GetGLProcAddress(LibGL, 'glEnable');
...
begin
  if not glStateArray[cap] then
  begin
    glEnable_C(cap);
    glStateArray[cap] := true;
  end;
end;

procedure glDisable(cap: GLenum);
begin
  if glStateArray[cap] then
  begin
    glDisable_C(cap);
    glStateArray[cap] := false;
  end;
end;
...
initialization
  for i := 0 to High(glStateArray) do
    glStateArray[i] := false;
